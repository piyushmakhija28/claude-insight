...
1. **Impact analysis is blind to the code it protects.** `parsers/config.py` caps the call-graph
   builder at 300 files against 411 on disk. The entire Level-2 SDLC core (45 files) and 38/45 hook
   files are 100% invisible to it; 25% of the budget burns on `tests/`. Fix before trusting any
   CallGraph-driven v2.0.0 decision.
