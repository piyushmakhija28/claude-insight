...
| **NFR-4** No silent regression | All 27 `capability_loss.md` capabilities carry a non-"disappeared" disposition **(COUNT CORRECTED 2026-08-02, owner ruling, was 25: the ledger holds 16 + 9 + 2 = 27 data rows)** | Script cross-checks the 27 names against **`docs/reports/capability-disposition-ledger.md`** -- NOT the policy audit matrix. **TARGET CORRECTED 2026-08-02:** this cell previously named the audit matrix, and that cross-check is structurally impossible -- the matrix's AC1 asserts row-set identity with `docs/policies/`, so any row added for a capability with no policy file (`daemon.py`, `registry.py`) fails it in both directions. Verified in a sandbox before the owner ruled for a second, capability-keyed ledger. **The join is by capability name, one row per `capability_loss.md` entry, matched STRICTLY, asserted as an empty symmetric difference in both directions rather than as a count.** Strict versus loose moved the unaccounted count from 20 to 2, so the choice is load-bearing and is recorded in the ledger's own "The join, stated explicitly" section. Gate: `scripts/verify_policy_capability_dispositions.py` |
...
opened); `capability_loss.md` (the 27 capabilities -- count corrected 2026-08-02, was 25 -- are referenced by count and by
