...
| **FR-9a (AC CORRECTED AND STRENGTHENED 2026-08-01, Phase 6 -- owner ruling. THIS ROW SUPERSEDES THE FR-9a ROW IMMEDIATELY BELOW, which is retained verbatim and must not be deleted.)** | **Why the previous AC was replaced, stated plainly.** It asserted on FOUR sites, one of which (`langgraph_engine/parsers/config.py:11`) is DEAD CODE read by nothing, and it OMITTED `langgraph_engine/parsers/graph_model.py:43` entirely -- a cap that BINDS and that survives any fix to the file cap. An implementer working strictly to the previous AC would have fixed four sites, one of them dead, watched every assertion pass, and left a binding truncation in production. The previous AC also asserted on *end states of constants*, and constant-inspection is precisely the failure mode that produced this defect: reading `config.py:11` and concluding the cap was found is exactly what 19 files across every phase did. **The two sites that bind, both verified on disk independently and MEASURED at runtime by `docs/phase-5-uml/callgraph_coverage_probe.md`:** (1) `langgraph_engine/parsers/call_graph_builder_legacy.py:64` -- `MAX_FILES = 300`, defaulted into `CallGraphBuilder.__init__` at `:76`, stored at `:79`, enforced at `:107` and `:118`; (2) `langgraph_engine/parsers/graph_model.py:43` -- `DEFAULT_MAX_PATHS = _env_int("CLAUDE_CG_MAX_PATHS", 500)`, applied at `:320-321`, enforced at `:354`, `:357` and `:388`, warning emitted at `:390-392`. **`config.py:11` is DROPPED from the assertion set.** It may be deleted as dead-code cleanup, but deleting it asserts nothing and must not be counted as progress. **Site accounting, so the count matches its enumeration:** 17 distinct truncation sites exist; 2 bind. The other 15 are: 1 dead file-count cap (`config.py:11`), 1 dormant duplicate with no importers (`sdlc_pipeline/architecture/00-code-graph-analysis/code_graph_analyzer.py:73`), 1 live-but-non-binding file-count cap above the corpus size and off the UML path (`scripts/architecture/03-execution-system/00-code-graph-analysis/code-graph-analyzer.py:68`, value 500 vs 411 files on disk), 4 file-size caps with MEASURED-ZERO impact (no source file in the repo exceeds 100 KB), 1 non-binding traversal cap (`graph_model.py:42` `DEFAULT_MAX_DEPTH = 30`; observed depths 11 and 7), 2 different-class truncators that do not participate in call-graph construction (`build_dependency_resolver/parsers.py:681-682`, `sdlc_pipeline/code_explorer.py:453`), and 5 downstream diagram truncators that sit after discovery. 1+1+1+4+1+2+5 = 15; 15+2 = 17. **MEASURABLE AC -- RUNTIME PROOF, not constant inspection. Every assertion below is on the OUTPUT of an actual in-process build, never on the value of a constant.** A committed regression test SHALL construct the builder against the project root and run `.build()`, then assert: **(A) Discovery is coverage-complete.** The set of files present in the builder's analysed set is EQUAL to the set of eligible source files independently enumerated by the test's own oracle -- set equality, empty symmetric difference, not a magic number. **(B) The named canary is whole.** The symmetric difference between the enumerated `langgraph_engine/sdlc_pipeline/` files and the analysed `langgraph_engine/sdlc_pipeline/` files is empty. MEASURED baseline 2026-08-01: that tree holds 45 files and the shipping builder analyses **0 of 45** -- the entire Level 2 SDLC Execution Core is invisible today. The 300-file budget is exhausted five files before the tree begins (first `sdlc_pipeline` file is at discovery index 304). **(C) No traversal truncation is emitted.** Log capture over `langgraph_engine.parsers.graph_model`'s logger during the build contains ZERO records matching `hit max_paths=` and `limit; results truncated`. **This assertion is the load-bearing one and the reason this correction exists:** that warning fired on BOTH probe runs, including the run with the file cap fully lifted. A fix that satisfies (A), (B) and (D) but not (C) is exactly the half-fix this correction was written to prevent, and must fail the gate. **(D) Regression floor, grounded in the MEASURED post-fix figures.** With the caps lifted, the probe measured `files_analyzed=411`, `total_classes=480`, `total_methods=3506` (also `functions=1340`, `call_edges=26114`, `resolved_edges=7004`). The test asserts `>= 411`, `>= 480`, `>= 3506` -- a floor, not equality, so that legitimately adding source files cannot fail the suite while any regression toward the truncated shipping figures (300 files / 449 classes / 2,844 methods) does. The exact 2026-08-01 values are recorded in the test as the documented baseline. **(E) The silent-no-op trap is closed.** The test SHALL NOT establish the fix by rebinding the module global `legacy.MAX_FILES`: the probe MEASURED that rebinding the module attribute is a **silent no-op** (still 300 files) because `max_files=MAX_FILES` binds at def-time, whereas the constructor kwarg and `__init__.__defaults__` patching both reach 411. A fix implemented as a module-global edit, or a test that "proves" the fix that way, fails this AC. **(F) `'fixed the one Phase 0 named' is still NOT an acceptable end state`** -- carried forward from the superseded AC, which was correct on this point. **SIZING IMPACT, FLAGGED NOT APPLIED:** `docs/phase-6-sprint/github_issues.json` issue V2-009 carries size 5 (`size_provenance: SOURCED`, WSJF 4.60), estimated against a two-site constant-change assumption. Assertions (A), (B), (C) and (E) add a runtime probe harness, an independent enumeration oracle, log capture, and a negative test for the def-time binding trap. **This document's judgement is that 5 no longer holds; it is NOT re-pointed here.** Re-sizing is product-manager-agent's call, per the same rule that kept FR-15 unsized. | Superseding AC written 2026-08-01; fix NOT started |
| FR-9a (Root-caused; fix not started -- **AC added at Phase 2.1, escalated from ADVISORY to required by the coordinator**) -- **SUPERSEDED 2026-08-01 by the row immediately above. RETAINED VERBATIM, NOT DELETED: other artifacts still reference this wording and a reader needs the disposition, not an absence.** | **Prior gap:** this row did not exist; FR-9a's only definition of done was the RTM's "(prerequisite for FR-10's AC)", which makes FR-9a's completion parasitic on FR-10's tests rather than independently checkable -- an implementer could fix the one site Phase 0 named, watch FR-10 go green, and leave three more truncation points intact. `hld.md` SS 12 OAQ 4 found FR-9a's true scope is **four** independent silent truncators, not one: (1) `langgraph_engine/parsers/config.py:11` (`MAX_FILES = 300`, consumed via `parsers/__init__.py:22` -- the one Phase 0 named); (2) `langgraph_engine/parsers/call_graph_builder_legacy.py:64` (`MAX_FILES = 300`, used at `:76`); (3) `langgraph_engine/sdlc_pipeline/architecture/00-code-graph-analysis/code_graph_analyzer.py:73` (`MAX_FILES = 500`, used at `:137`/`:154`/`:169` -- lives inside `sdlc_pipeline/`, the very package site 1 drops entirely); (4) `scripts/architecture/03-execution-system/00-code-graph-analysis/code-graph-analyzer.py:68` (`MAX_FILES = 500`, used at `:120`/`:137`/`:152` -- a near-duplicate of site 3 under a hyphenated filename). **Measurable AC:** each of the four sites above reaches exactly one of two recorded end states -- migrated to the ADR-013 coverage-complete contract (ENUMERATE -> ALLOCATE -> RECONCILE -> PROPAGATE, non-optional `DiscoveryManifest` constructor argument, default budget unbounded) or formally retired with the removal recorded (valid for sites 2 and 4 if no live caller remains); "fixed the one Phase 0 named" is explicitly NOT an acceptable end state. The ADR-013 regression test `test_discovery_covers_every_package` must pass, and its assertion 5 (the named canary) must confirm `manifest.packages["langgraph_engine/sdlc_pipeline"].analysed_n == 45` -- i.e. the package site 1 alone would silently drop is present in builder output at full count. **Explicitly out of scope, by decision not oversight:** a fifth truncator exists at `langgraph_engine/build_dependency_resolver/parsers.py:682` (`max_files_scanned = 1000`, used at `:696`) which returns a truncated `False` from a directory-detection helper rather than a truncated graph. `hld.md` OAQ 4 rules it a different defect class (a boolean, not a graph, and therefore not inspectable the way sites 1-4 are) and defers it to a v2.1 follow-on under ADR-013's general principle; it is not part of FR-9a's four-site closure requirement and its absence from the list above is deliberate. |
...
| FR-9a | Section 5 **superseding** row (Phase 6, 2026-08-01: runtime-proof AC replacing the four-site constant-inspection AC; the superseded row is retained directly beneath it) | C | **Two binding sites:** `langgraph_engine/parsers/call_graph_builder_legacy.py:64` (enforced `:107`, `:118`) and `langgraph_engine/parsers/graph_model.py:43` (`DEFAULT_MAX_PATHS`, warning at `:390-392`) + `test_discovery_covers_every_package` extended to runtime proof (set-equality discovery oracle, `sdlc_pipeline` canary, max_paths log-capture assertion, regression floor 411/480/3506, def-time-binding negative test). `langgraph_engine/parsers/config.py` is DROPPED from the assertion set as dead code -- optional cleanup only. *(Superseded, retained for traceability: the prior target was "Four sites: `parsers/config.py`, `parsers/call_graph_builder_legacy.py`, `sdlc_pipeline/architecture/00-code-graph-analysis/code_graph_analyzer.py`, `scripts/architecture/03-execution-system/00-code-graph-analysis/code-graph-analyzer.py`, each migrated or formally retired".)* |
...
| 2026-08-01 | 1.4 (Phase 6 -- FR-9a AC corrected and strengthened to runtime proof) | Owner ruling on a defect found by the Phase 6 scrum-master agent. FR-9a's AC named FOUR truncation sites, one of which (`parsers/config.py:11`) is dead code read by nothing, and OMITTED `parsers/graph_model.py:43` (`DEFAULT_MAX_PATHS`, default 500) -- a cap that binds and that survives fixing the file cap. An implementer working strictly to that AC would have watched every assertion pass and left a binding truncation in production. Added a superseding Section 5 row asserting on the TWO binding sites, dropping the dead one, and requiring RUNTIME PROOF rather than constant inspection: set-equality discovery oracle, the `sdlc_pipeline` 45-file canary (0 of 45 today), a log-capture assertion that no `hit max_paths=... limit; results truncated` record is emitted (the load-bearing assertion -- it fired on BOTH probe runs even with the file cap lifted), a regression floor of 411 files / 480 classes / 3,506 methods, and a negative test closing the silent-no-op trap where rebinding the module global does nothing because defaults bind at def-time. The superseded AC is RETAINED VERBATIM directly beneath the new row, with only its row label marked superseded -- other artifacts still reference that wording and a reader needs the disposition, not an absence. Section 6's RTM row re-pointed, with the prior target retained inline. 17 truncation sites exist in total; 2 bind; the 15 non-binding are enumerated in the new row so the count matches its list. Figures sourced from `docs/phase-5-uml/callgraph_coverage_probe.md` (MEASURED-RUNTIME); both binding sites re-verified on disk by this pass. **Sizing flagged, not applied:** V2-009's 5 points were estimated on a two-site constant-change assumption and this pass judges 5 no longer sufficient; re-sizing is product-manager-agent's call. Mirrored into `SRS.md` as an appended revised AC-21 (rules/44 section 4.2) and into `docs/phase-6-sprint/github_issues.json` V2-009. | Corrected; fix not started |
