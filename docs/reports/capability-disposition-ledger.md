...
| 2 | Skill/agent-selection-pending block | `hooks/pre_tool_enforcer/policies/skill_selection.py` |  FR-9 |  | UNDECIDED. Missing: no row of the 46-policy matrix cites this file, and none of the 46 corpus policies claims this gate; HLD SS 12 OAQ 2's 15 hook-coupled dispositions include no skill-selection policy. MEASURED 2026-08-02: the gate is live -- `check_skill_selection_pending` blocks Write/Edit/NotebookEdit at `hooks/pre_tool_enforcer/policies/skill_selection.py:12`, registered `core.py:456` `_BLOCKING_POLICIES` -- so this is working capability, not a dead reference. Decider: solution-architect, as an OAQ 2 extension | MEASURED |
| 3 | Review-checkpoint block (blocks code-changing tools mid-review) | `hooks/pre_tool_enforcer/policies/checkpoint.py` | FR-9 |  | UNDECIDED. Missing: as row 2 -- no matrix row cites this file, no corpus policy claims it, OAQ 2 does not reach it. MEASURED 2026-08-02: live and blocking -- `check_checkpoint_pending` at `hooks/pre_tool_enforcer/policies/checkpoint.py:11`, first entry of `core.py:454` `_BLOCKING_POLICIES`. Decider: solution-architect, as an OAQ 2 extension | MEASURED |
...
| 6 | Windows-only-shell-command detection + branch protection | `hooks/pre_tool_enforcer/policies/bash_commands.py` | FR-9, NFR-5 |  | UNDECIDED. Missing: the nearest matrix row, 46 (`windows-path-policy.md`, keep-as-is), rests on `langgraph_engine/preflight_guard/nodes.py:240-323` `node_windows_path_check` -- a path check run once at pipeline start, not a per-tool-call shell-command block -- so it cannot be read as covering this capability; and nothing anywhere disposes the branch-protection half, which no corpus policy names at all. Decider: solution-architect, and the two halves may need separate dispositions | CITED |
| 7 | Python/Windows cp1252 Unicode-crash detection | `hooks/pre_tool_enforcer/policies/python_unicode.py` | FR-9, NFR-5 |  | UNDECIDED. Missing: a ruling on whether Level 0's kept enforcement points -- matrix rows 43 (`unicode-fix-policy.md`) and 13 (`encoding-validation-policy.md`), both keep-as-is on `langgraph_engine/preflight_guard/nodes.py:62-147` and `:150-237` -- are accepted as covering this capability. They fire at pipeline start on files already written; this one blocks the Write/Edit before the crash-inducing content lands. Different trigger, different failure mode, and no artifact judges them equivalent. Decider: solution-architect | CITED |
...
| 12 | General-purpose subagent persona gate | `hooks/pre_tool_enforcer/policies/agent_persona.py` | FR-9 |  | UNDECIDED. Missing: no corpus policy and no ADR or OAQ row names this gate. It enforces a user-level convention -- that a generic subagent dispatch must carry an injected `---persona---` block from the agent library -- rather than a project policy, so the question of whether it survives as a plugin agent-instruction, ports to a gate, or is dropped is not one the existing artifacts answer. MEASURED 2026-08-02: live and blocking -- `check_agent_persona` is the final entry of `core.py:453-469` `_BLOCKING_POLICIES`. Decider: project owner, since the convention it enforces is the owner's own | MEASURED |
| 13 | Dynamic skill/agent context injection by file type | `hooks/pre_tool_enforcer/policies/skill_context.py` | FR-9 |  | UNDECIDED. Missing: no corpus policy names it. MEASURED 2026-08-02 that it is hint-only and never blocks (`hooks/pre_tool_enforcer/policies/skill_context.py:3`; invoked outside `_BLOCKING_POLICIES` at `core.py:497-502`), which is the shape OAQ 2 row 2 sends to the plugin's agent-instruction layer -- but OAQ 2 applied that criterion only to policies it named, and extending it to an unnamed mechanism is an inference this pass will not make on the owner's behalf. Decider: solution-architect, as an OAQ 2 extension | MEASURED |
...
| 21 | Skill-selection flag clearing (Level 3.5) | `hooks/post_tool_tracker/policies/skill_selection_clear.py` | FR-9 |  | UNDECIDED, and dependent. MEASURED 2026-08-02: this module exists only to clear the flag row 2's gate sets -- `handle_skill_selection` at `hooks/post_tool_tracker/policies/skill_selection_clear.py:12` clears `.skill-selection-pending`. It has no independent purpose, so it cannot be dispositioned before row 2 is: a gate that ports needs its clearing half ported with it, a gate that is deleted takes this with it. Decider: whoever decides row 2, in the same ruling | MEASURED |
...
| 25 | Auto-close GitHub issues on task completion | `hooks/post_tool_tracker/github_integration.py` | FR-4 (integration lifecycle) |  | UNDECIDED. Missing: an owning policy that exists. MEASURED 2026-08-02: the module declares `Policy: github-integration-policy.md` at `hooks/post_tool_tracker/github_integration.py:46`, and no such file is in the 46-policy corpus. The two nearest corpus names are separate matrix rows with the same disposition but different scope -- row 19 (`github-issues-integration-policy.md`, keep-as-is, reach=False) and row 25 (`issue-closure-policy.md`, keep-as-is, `github_lifecycle.py:543` `step6_close_issue`) -- and both rest on pipeline-side closure at a Step 6 boundary, a different trigger from closing on post-tool completion detection. Deciding which row owns this, or that neither does, is not a call this pass can make from the evidence. Decider: solution-architect | MEASURED |
