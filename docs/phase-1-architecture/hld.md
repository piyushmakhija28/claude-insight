...
| **NFR-4** No silent regression | All 27 `capability_loss.md` capabilities carry a non-"disappeared" disposition **(COUNT CORRECTED 2026-08-02, owner ruling, was 25: the ledger holds 16 + 9 + 2 = 27 data rows)** | Script cross-checks the 27 names against the audit matrix; fails on any missing or empty cell. The PreToolUse table is **16 PreToolUse components (14 policy gates plus the daemon and registry mechanism)** -- the superseded 25 dropped the `daemon.py` (NFR-1) and `registry.py` (FR-9) rows | Workstream A | Precondition satisfied; SS 12 OAQ 2 supplies the 15 hook-coupled dispositions |
