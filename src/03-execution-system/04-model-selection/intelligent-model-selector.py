...
class IntelligentModelSelector:
    ...
    def select_model(
        ...
        """
        print("=" * 80)
        print("[ROBOT] INTELLIGENT MODEL SELECTION")
        print("=" * 80)
        ...
        if plan_mode_decision.get('auto_enter') or plan_mode_decision.get('in_plan_mode'):
            ...
            print(f"\n[RED] PLAN MODE DETECTED -> OPUS (Mandatory)")
        ...
        if risk_factors:
            print(f"\n[WARNING]️  Risk Factor Analysis:")
            ...
            if security_risk:
                ...
                if selection['selected_model'] == 'HAIKU':
                    selection['selected_model'] = 'SONNET'
                    selection['reasoning'].append('[LOCK] Security-critical: upgraded to SONNET')

            if multi_service_risk:
                print(f"   - Multi-service impact detected")
                if selection['selected_model'] == 'HAIKU':
                    selection['selected_model'] = 'SONNET'
                    selection['reasoning'].append('[CYCLE] Multi-service coordination: upgraded to SONNET')
...
def main():
    ...
    # Use defaults for testing if no arguments provided
    if len(sys.argv) < 4:
        print("=" * 80)
        print("Intelligent Model Selector")
        print("=" * 80)

        # If no args at all, run with test defaults
        if len(sys.argv) == 1:
            print("\n[INFO] Running with test defaults...")
            complexity = {
                'score': 8,
                'level': 'MODERATE',
            ...
            print("\nNote: Run without arguments to use test defaults")
            sys.exit(1)
    else:
