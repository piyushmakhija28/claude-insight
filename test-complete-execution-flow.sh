...
echo "   Result: Context at ${CONTEXT_PCT2}% ($(python -c "print('Apply optimizations' if float('$CONTEXT_PCT2') > 70 else 'OK')"))"
echo
...
if [ -f ~/.claude/memory/03-execution-system/05-skill-agent-selection/auto-skill-agent-selector.py ]; then
    # Create properly formatted input files
    cd ~/.claude/memory

    # Use existing test files (already created with correct data)
