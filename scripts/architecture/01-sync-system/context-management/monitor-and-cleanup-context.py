...
def get_actual_context():
    ...
    if not os.path.exists(tracking_file):
        print("[WARNING]️  No context tracking data found")
    ...
    try:
        ...
        print(f"[CROSS] Error reading context data: {e}", file=sys.stderr)
        return None
...
def determine_cleanup_action(context_percent):
    ...
    if context_percent >= THRESHOLDS["aggressive"]:
        return {
            ...
            "message": "[ALERT] CRITICAL: Full compact required!",
        ...
        return {
            ...
            "message": "[WARNING]️  HIGH: Auto-compact recommended",
            "protect_session": True,
        }
        ...
        return {
            ...
            "message": "[BULB] MEDIUM: Light cleanup suggested",
            "protect_session": True,
        }
    else:
        return {
            ...
            "action": "none",
            "command": None,
            "message": "[CHECK] OK: No cleanup needed",
...
def trigger_cleanup(action_info, dry_run=False):
    ...
    try:
        ...
        if action_info["protect_session"]:
            print("\n[SHIELD]️  Session memory will be PROTECTED")
...
def monitor_context(auto_trigger=False, dry_run=False):
    ...
    print("\n" + "=" * 70)
    print("[SEARCH] CONTEXT MONITOR - ACTUAL USAGE")
    print("=" * 70)
