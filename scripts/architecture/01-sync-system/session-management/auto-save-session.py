...
def save_session_summary(project_name, summary):
    ...
    try:
        ...
        print(f"[CHECK] Session saved: {session_file}")
        log_policy_hit("session-saved", f"project={project_name}, file={session_file}")

    except Exception as e:
        print(f"[CROSS] Failed to save session: {e}", file=sys.stderr)
    ...
    if not os.path.exists(project_summary_file):
        try:
            ...
            print(f"[CHECK] Project summary created: {project_summary_file}")
        ...
        try:
            ...
            print(f"[CHECK] Project summary updated: {project_summary_file}")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("[FLOPPY] AUTO-SAVE SESSION SUMMARY")
    print("=" * 70)
    print(f"\nProject: {args.project}")

    # Generate summary
    print("\n[U+1F4DD] Generating session summary...")
    ...
    if args.dry_run:
        ...
        # Save summary
        print("\n[FLOPPY] Saving session summary...")
        if save_session_summary(args.project, summary):
            print("\n[CHECK] Session summary saved successfully!")
        else:
            print("\n[CROSS] Failed to save session summary")
            sys.exit(1)
