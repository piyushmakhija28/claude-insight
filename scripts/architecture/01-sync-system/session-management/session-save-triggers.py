...
def main():
    ...
    if args.json:
        ...
        print("\n" + "=" * 70)
        print("[CHART] SESSION SAVE TRIGGER DETECTION")
        print("=" * 70)
        print(f"\nProject: {result['project']}")
        print(f"Timestamp: {result['timestamp']}")

        if result['should_save']:
            print(f"\n[CHECK] AUTO-SAVE RECOMMENDED ({result['trigger_count']} triggers met)")
            ...
            print("\n[FLOPPY] Action: Auto-save session summary")

        else:
            print("\n[PAUSE]️  NO TRIGGERS MET")
