...
def main():
    ...
    if args.json:
        ...
        if not detected:
            print("\n[WARNING]️  No preferences detected in recent logs")
            print("\n[BULB] Preferences are detected from conversation patterns")
        else:
            ...
            for category, values in detected.items():
                ...
                for value, val_count in sorted(values.items(), key=lambda x: x[1], reverse=True):
                    ...
                    learned = "[CHECK] LEARNED" if category in existing and existing[category] == value else ""
            ...
            print("\n" + "=" * 70)
            print("[FLOPPY] Existing Learned Preferences:")

            if existing:
                for category, value in existing.items():
                    print(f"   [CHECK] {category.replace('_', ' ').title()}: {value}")
            else:
