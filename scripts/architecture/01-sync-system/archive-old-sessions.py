    ...
    for month_key, month_sessions in sessions_by_month.items():
        ...
        with tarfile.open(archive_file, f'{mode}:gz') as tar:
            for session in month_sessions:
                ...
                print(f"  ✓ Archived: {session['name']} → {month_key}/sessions.tar.gz (age: {session['age_days']} days)")
                archived_count += 1
...
def archive_project(project_name, dry_run=False):
    ...
    if not sessions_to_archive:
        print(f"✓ {project_name}: All sessions are recent (last {len(session_files)} sessions kept)")
        return 0
...
def archive_all_projects(dry_run=False):
    ...
    print(f"🗂️  {mode_str}Archiving sessions for {len(projects)} projects...")
    ...
    if total_archived > 0:
        if dry_run:
            print(f"📊 Would archive {total_archived} sessions")
        else:
            print(f"✅ Archived {total_archived} sessions successfully!")
            log_action('archived-all', f'{total_archived} total sessions')
    else:
        print("✓ No sessions need archiving (all are recent)")
