...
**VERSION:** 2.0.0
**CREATED:** 2026-02-16
**UPDATED:** 2026-02-22 — Always-Task Policy (complexity-based se hata diya)
...
1. ✅ **ALWAYS create tasks** — complexity nahi dekhna, har request pe TaskCreate
2. ✅ **Break down** into phases (jab 5+ tasks ho jayein)
3. ✅ **Create tasks** minimum 1 task per request
4. ✅ **Auto-track** status as Claude works
5. ✅ **Auto-update** progress without manual intervention

**NEW RULE (v2.0.0):** Task creation is NEVER skipped based on complexity.
Purpose: Policy visibility — user dekhta hai ki system kaam kar raha hai.
...
def analyze_complexity(structured_prompt: Dict) -> Dict:
    ...
    return {
        ...
        'needs_phases': complexity_score >= 10,  # 10+ = phases
        'needs_tasks': True,  # ALWAYS True — har request pe task banana
        'estimated_tasks': max(files_count, 1)   # Minimum 1 task
    }
...
|-------|-------|---------|--------|----------|
| 0-4 | SIMPLE | No | YES (min 1) | 1 task — always create |
...
NOTE: Tasks column is ALWAYS YES now (v2.0.0 change).
Phase trigger changed to: 5+ tasks = phases (was complexity >= 10).
