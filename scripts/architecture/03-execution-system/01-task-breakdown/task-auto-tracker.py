...
class TaskAutoTracker:
    ...
    def _handle_bash(self, task_id: str, params: Dict, result: Any):
        ...
        if any(cmd in command for cmd in ['mvn clean install', 'mvn compile', 'mvn package']):
            ...
            if result and 'BUILD SUCCESS' in str(result):
                self.update_task_progress(
                    task_id,
                    current_step="Build successful [CHECK]",
                ...
                self.update_task_progress(
                    task_id,
                    current_step="Build failed [CROSS]",
                    progress_increment=0,
        ...
        # Detect test commands
            ...
            if result and any(keyword in str(result) for keyword in ['PASS', 'SUCCESS', 'OK']):
                self.update_task_progress(
                    task_id,
                    current_step="Tests passed [CHECK]",
                    progress_increment=15,
        ...
        # Detect curl/API testing
            ...
            if result and '200' in str(result):
                self.update_task_progress(
                    task_id,
                    current_step=f"Endpoint working [CHECK]",
                    progress_increment=10,
    ...
    def auto_complete_task(self, task_id: str):
        ...
        self.log(f"[CHECK] Auto-completed task: {task_id} - {task['subject']}")
    ...
    def on_phase_complete(self, phase_name: str):
        ...
        """
        self.log(f"[CHECK] PHASE COMPLETE: {phase_name}")
        print(f"\n{'='*80}")
        print(f"[CHECK] PHASE COMPLETE: {phase_name}")
        print(f"{'='*80}\n")
    ...
    def unlock_dependent_tasks(self, completed_task_id: str):
        ...
        for task_id, task in self.active_tasks.items():
            ...
            if completed_subject in dependencies:
                ...
                if not dependencies:
                    task['blocked'] = False
                    self.log(f"[U+1F513] Unlocked task: {task_id} - {task['subject']}")
...
def start_monitoring():
    ...
    """
    print("[ROBOT] Starting Task Auto-Tracker...")
    ...
    print(f"[CHECK] Monitoring: {tracker.tool_call_log}")
    print("[CHECK] Auto-tracking enabled")
    ...
    try:
        ...
        observer.stop()
        print("\n[U+1F6D1] Stopping Task Auto-Tracker...")
