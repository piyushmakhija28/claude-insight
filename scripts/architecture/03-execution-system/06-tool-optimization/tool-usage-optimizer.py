...
class ToolUsageOptimizer:
    ...
    def optimize(
        ...
        print(f"\n{'='*60}")
        print(f"[SEARCH] TOOL OPTIMIZATION: {tool_name}")
        print(f"{'='*60}")
        ...
        if savings > 0:
            print(f"\n[MONEY] Estimated Token Savings: ~{savings}%")
    ...
    def optimize_bash(self, params: Dict, context: Dict) -> Dict:
        ...
        # [U+1F333] TREE PATTERN: Suggest tree for structure understanding
        if context.get('first_time_in_directory'):
            directory = context.get('directory', '')
            if directory and 'tree' not in command:
                suggestion = f"find {directory} -maxdepth 3 -type d ! -path '*/\\.*' | sort"
                print(f"\n[BULB] SUGGESTION: Use tree to understand structure first:")
                print(f"   {suggestion}")
        ...
        if 'tree' in command:
            ...
            if context.get('file_type') and '-P' not in command:
                ...
                print(f"   [BULB] Consider: tree -P '*.{file_type}' for specific files")
