...
try:
    ...
    # Load prompt-generator.py
    spec = importlib.util.spec_from_file_location(
        "prompt_generator",
        MEMORY_PATH / '03-execution-system' / '00-prompt-generation' / 'prompt-generator.py'
    )
    prompt_generator = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(prompt_generator)

    # Load blocking-policy-enforcer.py
    spec = importlib.util.spec_from_file_location(
...
class PromptAutoGenerator:
        ...
        if result.get('success'):
            try:
                if DIRECT_IMPORT_AVAILABLE:
                    ...
                    enforcer.mark_complete('prompt_generated')
                else:
                    # Fallback to subprocess
                    import subprocess
                    subprocess.run(
    ...
    def print_result(self, result):
        ...
        print(f"\n{'='*70}")
        print(f"[ROBOT] Auto-Prompt Generation (Step 0)")
        print(f"{'='*70}\n")
        ...
        if result.get('skip'):
            ...
            print(f"[TARGET] Intent: {result.get('intent', 'unknown').upper()}")
            print(f"[LOG] Original: {result.get('original_message')[:100]}...")
