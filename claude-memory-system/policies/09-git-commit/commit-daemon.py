...
def main():
    ...
    if args.stop:
        if is_daemon_running():
            try:
                ...
                print(f"[CHECK] Commit daemon stopped (PID: {pid})")
            except Exception as e:
                print(f"[CROSS] Failed to stop daemon: {e}")
        else:
            print("[INFO]️  Commit daemon is not running")
        return
    ...
    if args.status:
        if is_daemon_running():
            with open(DAEMON_PID_FILE, 'r') as f:
                pid = f.read().strip()
            print(f"[CHECK] Commit daemon is running (PID: {pid})")
            print(f"   Project: {args.project_dir}")
            print(f"   Checking every {args.interval} minutes")
        else:
            print("[WARNING]️  Commit daemon is not running")
        return

    # Check if already running
    if is_daemon_running():
        print("[WARNING]️  Commit daemon is already running!")
    ...
    print("=" * 70)
    print("[ROCKET] GIT AUTO-COMMIT DAEMON STARTING")
    print("=" * 70)
    ...
    print("\n[BULB] To stop: python commit-daemon.py --stop")
    print("[BULB] To check status: python commit-daemon.py --status")
