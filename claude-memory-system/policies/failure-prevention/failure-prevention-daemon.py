...
def main():
    ...
    if args.stop:
        if is_daemon_running():
            try:
                ...
                print(f"[CHECK] Failure prevention daemon stopped (PID: {pid})")
            except Exception as e:
                print(f"[CROSS] Failed to stop daemon: {e}")
        else:
            print("[INFO]️  Failure prevention daemon is not running")
        return
    ...
    if args.status:
        if is_daemon_running():
            with open(DAEMON_PID_FILE, 'r') as f:
                pid = f.read().strip()
            print(f"[CHECK] Failure prevention daemon is running (PID: {pid})")
            ...
            print("[WARNING]️  Failure prevention daemon is not running")
        return

    # Handle --run-now
    if args.run_now:
        print("[SEARCH] Running failure detection and learning manually...")
        ...
        success, output = run_failure_detection()
        if not success:
            print(f"   [CROSS] Detection failed: {output}")
            return
        ...
        success, output = run_failure_learning(project_name)
        if not success:
            print(f"   [CROSS] Learning failed: {output}")
            return
    ...
    if is_daemon_running():
        print("[WARNING]️  Failure prevention daemon is already running!")
    ...
    print("=" * 70)
    print("[ROCKET] FAILURE PREVENTION DAEMON STARTING")
    print("=" * 70)
    ...
    print("\n[BULB] To stop: python failure-prevention-daemon.py --stop")
    print("[BULB] To check status: python failure-prevention-daemon.py --status")
    print("[BULB] To run manually: python failure-prevention-daemon.py --run-now")
