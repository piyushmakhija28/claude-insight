...
# Scripts in context_sync/architecture/ are not a Python package.
# Use importlib.util.spec_from_file_location for dynamic import.
# ============================================================================


def _load_architecture_script(script_name):
    """Dynamically load a script from context_sync/architecture/.
    ...
    if script_path is None:
        for base in (arch_dir, legacy_dir):
            ...
            if matches:
                ...
                break
    if script_path is None:
        _ARCH_LOADER_LOGGER.warning(f"context_sync architecture script not found: {script_name}")
        return None

    try:
        spec = importlib.util.spec_from_file_location(stem, str(script_path))
        if spec is None or spec.loader is None:
            _ARCH_LOADER_LOGGER.warning(f"context_sync architecture script has no import spec: {script_path}")
            return None
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return module
    except Exception as exc:  # noqa: BLE001 - plugin boundary: an optional enhancement must never crash the pipeline
        _ARCH_LOADER_LOGGER.warning(f"context_sync architecture script failed to import {script_path}: {exc}")
        return None
