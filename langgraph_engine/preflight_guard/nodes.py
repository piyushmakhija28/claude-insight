"""Level 0 (Pre-Flight Sanity Guard) Auto-Fix check node functions.
...
def node_unicode_fix(state: FlowState) -> dict:
    ...
    _logger.debug("[L0 UNICODE FIX] state['project_root'] at entry: '%s'", state.get("project_root", "MISSING"))
    updates = {}
    try:
        ...
        if sys.platform != "win32":
            ...
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "Unicode UTF-8 Fix", True, "Not Windows platform"
            )
            write_level_log(state, "level-minus1", "unicode-fix", "SKIP", time.time() - _step_start, updates)
            return updates
        ...
        if applied:
            ...
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "Unicode UTF-8 Fix", True, "UTF-8 encoding applied"
            )
        else:
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "Unicode UTF-8 Fix", True, "Already UTF-8 configured"
            )
        _logger.debug("[L0 UNICODE FIX] Returning: %s", list(updates.keys()))
        ...
        _logger.debug("[L0 UNICODE FIX] Returning (exception): %s", list(updates.keys()))
...
def node_encoding_validation(state: FlowState) -> dict:
    ...
    try:
        if sys.platform != "win32":
            ...
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "ASCII-only Python files", True, "Not Windows platform"
            )
        ...
        if non_ascii_files:
            ...
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "ASCII-only Python files", False, updates["encoding_check_error"]
            )
        else:
            updates["encoding_check"] = True
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "ASCII-only Python files", True, "All files ASCII-safe"
            )
...
def node_windows_path_check(state: FlowState) -> dict:
    ...
    try:
        if sys.platform != "win32":
            ...
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "Windows path handling", True, "Not Windows platform"
            )
        ...
        if issues:
            ...
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "Windows path handling", False, updates["windows_path_check_error"]
            )
        else:
            updates["windows_path_check"] = True
            logger and logger.log_validation_result(
                "Level 0 (Pre-Flight Sanity Guard)", "Windows path handling", True, "No backslash paths found"
            )
