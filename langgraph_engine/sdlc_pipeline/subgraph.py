"""
SDLC Execution Core (Level 2) SubGraph - Integrated Execution Pipeline (Steps 0-8)

Bridge module that wraps the WORKFLOW.md-compliant sdlc_pipeline.py functions
with proper orchestrator integration and logging.

Current scheme: Step 0 (Pre-Analysis & CallGraph Scan) does not route through
_run_step below -- its two sub-phase nodes (step0_project_context_node,
step0_callgraph_snapshot_node) are plain LangGraph nodes. _run_step handles
Steps 1-8 (Task Orchestration through Final Telemetry & Summary Report).
...
def _run_step(
    ...
    *,
    ...
    Args:
        step_number:    Numeric step (1-8), used for checkpoint naming.
                         Step 0 (Pre-Analysis) does not route through here.
    ...
    # Dry-run: skip Steps 2-8 (issue tracking, implementation, PR, review, docs, summary)
    if step_number >= 2 and _os.environ.get("CLAUDE_DRY_RUN") == "1":
    ...
    # Track pipeline-level start time when Step 1 (Task Orchestration) begins
    if step_number == 1:
        try:
    ...
    # Only for steps that run external commands/tools (Steps 1-4: Task
    # Orchestration, Issue Tracking, Branch Setup, Implementation)
        ...
        if step_number == 8 and result is not None:
            try:
                ...
                if _pipeline_start is not None:
                    ...
                    result["sdlc_total_execution_time_ms"] = round(total_time, 1)
...
# ============================================================================
# STEP NODE WRAPPERS - Extracted to sdlc_pipeline/nodes/ package
# ============================================================================
