...
def step0_project_context_node(state: FlowState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "step0_project_context": context,
            "step0_files_read": files_read,
            "step0_project_context_time_ms": round(elapsed, 1),
        }
    except Exception as e:
        elapsed = (_t.time() - _start) * 1000
        return {
            "step0_project_context": {},
            "step0_files_read": [],
            "step0_project_context_error": str(e),
            "step0_project_context_time_ms": round(elapsed, 1),
        }


def step0_callgraph_snapshot_node(state: FlowState) -> Dict[str, Any]:
    ...
    try:
        ...
        elapsed = (_t.time() - _start) * 1000
        return {
            "step0_callgraph_snapshot": snapshot,
            "step0_callgraph_available": bool(snapshot),
            "step0_callgraph_time_ms": round(elapsed, 1),
        }
    except Exception as e:
        elapsed = (_t.time() - _start) * 1000
        logger.debug("[v2] Initial callgraph snapshot skipped: %s" % str(e))
        return {
            "step0_callgraph_snapshot": None,
            "step0_callgraph_available": False,
            "step0_callgraph_error": str(e),
            "step0_callgraph_time_ms": round(elapsed, 1),
        }


def sdlc_init_node(state: FlowState) -> Dict[str, Any]:
    """
...
class StepNodeFactory:
    ...
    Usage::
        ...
        node = StepNodeFactory.make(
            step_number=2,
            step_label="Issue Tracking",
            step_fn=step2_issue_tracking,
            fallback={"step2_status": "ERROR"},
        )
        graph.add_node("sdlc_step2_issue_tracking", node)
    ...
    def make(step_number, step_label, step_fn, fallback=None):
        ...
        Args:
            step_number: Numeric step index (0-8).
