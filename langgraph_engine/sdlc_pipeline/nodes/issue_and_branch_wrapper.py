...
CHANGE LOG (v1.13.0):
  ...
  _build_retry_history_context is kept (consumed by step4_implementation_note).
"""
...
try:
    from .issue_and_branch_implementation import step2_github_issue_creation, step3_branch_creation
except ImportError as _imp_err:  # pragma: no cover
    logger.error("issue_and_branch_implementation import failed: {}", _imp_err)

    def step2_github_issue_creation(state):  # type: ignore[misc]
        """Fallback stub used when issue_and_branch_implementation import fails."""
        return {
            "step2_issue_id": "0",
            "step2_issue_url": "",
            "step2_issue_created": False,
            "step2_status": "ERROR",
            "step2_error": "issue_and_branch_implementation unavailable",
        }

    def step3_branch_creation(state):  # type: ignore[misc]
        """Fallback stub used when issue_and_branch_implementation import fails."""
        return {
            "step3_branch_name": "",
            "step3_branch_created": False,
            "step3_status": "ERROR",
            "step3_error": "issue_and_branch_implementation unavailable",
        }


# REMOVED: step1_skill_selection_node -- collapsed into Step 0 template (v1.13.0)
# REMOVED: step1_skill_validation_node -- collapsed into Step 0 template (v1.13.0)
# REMOVED: step1_final_prompt_node -- collapsed into Step 0 template (v1.13.0)
#
# Outputs these nodes used to produce are now written by step1_task_analysis_node
# after the orchestration template LLM call.


def step2_github_issue_node(state: FlowState) -> Dict[str, Any]:
    ...
    def _with_network_retry(st):
        ...
        for attempt in range(3):
            try:
                return step2_github_issue_creation(st)
    ...
    if os.environ.get("ENABLE_JIRA", "0") == "1":
        try:
            ...
            jira_result = jira_wf.step2_create_jira_issue(
                title=result.get("step2_title", ""),
                description=state.get("user_message", ""),
                label=result.get("step2_label", "task"),
                github_issue_url=result.get("step2_issue_url", ""),
                github_issue_number=int(result.get("step2_issue_id", "0")),
            )
...
def step3_branch_creation_node(state: FlowState) -> Dict[str, Any]:
    ...
    def _with_network_retry(st):
        ...
        if jira_key and st.get("jira_issue_created", False):
            label = st.get("step2_label", "feature")
            jira_branch = "{}/{}".format(label, jira_key.lower())
            # Inject override into a mutable copy so the underlying step
            # picks it up via state["step3_branch_name"] if it checks that field,
            # or via a dedicated override key that step3_branch_creation reads.
            st = dict(st)
            st["step3_jira_branch_override"] = jira_branch
            logger.info("[v2] Jira branch override: {}", jira_branch)

        last_exc = None
        for attempt in range(3):
            try:
                return step3_branch_creation(st)
            except Exception as exc:
    ...
    return _run_step(
        ...
        state,
        fallback_result={
            "step3_branch_name": "fallback-branch",
            "step3_branch_created": False,
            "step3_status": "ERROR",
        },
    )
...
def _build_retry_history_context(state) -> str:
    ...
    retry_messages = state.get("step5_retry_messages", [])
    review_issues = state.get("step5_review_issues", [])
    max_retries = 3
