...
implementation_and_review_wrapper.py invoke.
...
def step4_implementation_execution(state: FlowState) -> Dict[str, Any]:
    ...
    Returns a well-formed result dict with step4_* keys.  On any failure
    ...
    State keys consumed:
        ...
        step1_execution_prompt      - base implementation prompt (fallback)
        step4_execution_prompt     - override from wrapper if set
        step5_review_issues        - populated on retry attempts

    State keys produced (always present in return dict):
        step4_implementation_status  - "SUCCESS" | "ERROR"
        step4_tasks_executed         - 1 on success, 0 on error
        step4_modified_files         - list[str]
        step4_llm_invoked            - bool
        step4_system_prompt_loaded   - bool
        step4_user_message_loaded    - bool
        step4_execution_time_ms      - float
        step4_llm_response           - str (raw LLM text)
        step4_changes_summary        - dict (files_modified, count, length)
        step4_error                  - str (empty on success)
    """
    step_start = time.time()

    result: Dict[str, Any] = {
        "step4_implementation_status": "ERROR",
        "step4_tasks_executed": 0,
        "step4_modified_files": [],
        "step4_llm_invoked": False,
        "step4_system_prompt_loaded": False,
        "step4_user_message_loaded": False,
        "step4_execution_time_ms": 0.0,
        "step4_llm_response": "",
    ...
    try:
        ...
        if system_prompt_path.exists():
            try:
                ...
                result["step4_system_prompt_loaded"] = True
        ...
        if user_message_path.exists():
            try:
                ...
                result["step4_user_message_loaded"] = True
        ...
        if not user_message:
            ...
            user_message = state.get("step4_execution_prompt") or state.get("step1_execution_prompt") or ""
            if user_message:
                result["step4_user_message_loaded"] = True
        ...
        if system_prompt and user_message:
            ...
            result["step4_error"] = "No prompt available: system_prompt.txt missing and step1_execution_prompt empty"
            result["step4_execution_time_ms"] = (time.time() - step_start) * 1000
            logger.error("[Step10] {}", result["step4_error"])
            return result
        ...
        if not llm_response:
            result["step4_error"] = "llm_call returned None or empty response"
            result["step4_execution_time_ms"] = (time.time() - step_start) * 1000
            logger.error("[Step10] {}", result["step4_error"])
            return result

        result["step4_llm_response"] = llm_response
        logger.info("[Step10] LLM response received ({} chars)", len(llm_response))

        # --- Persist response to disk ---
        try:
            response_path = Path(session_dir) / "step4_llm_response.txt"
        ...
        result["step4_modified_files"] = modified_files
        ...
        result["step4_changes_summary"] = changes_summary

        # --- Mark success ---
        result["step4_implementation_status"] = "SUCCESS"
        result["step4_tasks_executed"] = 1
        result["step4_error"] = ""

    except Exception as exc:
        result["step4_error"] = str(exc)
        result["step4_implementation_status"] = "ERROR"
        logger.exception("[Step10] Unexpected error: {}", exc)

    result["step4_execution_time_ms"] = (time.time() - step_start) * 1000
    logger.info(
        "[Step10] Done: status={}, files={}, elapsed={:.0f}ms",
        result["step4_implementation_status"],
        len(result["step4_modified_files"]),
        result["step4_execution_time_ms"],
    )
    return result
...
def step5_pull_request_review(state: FlowState) -> Dict[str, Any]:
    """Step 11: Pull Request creation and code review via Level3GitHubWorkflow.

    Reads branch/issue context from state, delegates to
    Level3GitHubWorkflow.step5_create_pull_request(), and maps the result
    to step5_* keys.
    ...
    State keys consumed:
        ...
        step2_issue_id                - GitHub issue number
        step3_branch_created          - gate: skip if False/missing
        step3_branch_name             - source branch for PR
        step4_modified_files         - list of changed files for summary
        step4_implementation_summary - optional narrative summary
        step4_changes_summary        - dict with files_modified list
        step1_selected_skills         - passed through to PR creation
        step1_selected_agents         - passed through to PR creation

    State keys produced (always present in return dict):
        step5_pr_id          - str (PR number)
        step5_pr_url         - str
        step5_review_passed  - bool
        step5_review_issues  - list[str]
        step5_merged         - bool
        step5_status         - "OK" | "ERROR" | "SKIPPED"
        step5_execution_time_ms - float
        step5_error          - str (empty on success)
    """
    step_start = time.time()
    ...
    result: Dict[str, Any] = {
        "step5_pr_id": "0",
        "step5_pr_url": "",
        "step5_review_passed": True,
        "step5_review_issues": [],
        "step5_merged": False,
        "step5_status": "ERROR",
        "step5_execution_time_ms": 0.0,
        "step5_error": "",
    }

    try:
        # --- Gate: branch must exist ---
        if not state.get("step3_branch_created", False):
            result["step5_status"] = "SKIPPED"
            result["step5_error"] = "step3_branch_created is False; skipping PR creation"
            logger.warning("[Step11] {}", result["step5_error"])
            result["step5_execution_time_ms"] = (time.time() - step_start) * 1000
            return result

        # --- Gate: Level3GitHubWorkflow must be importable ---
        if Level3GitHubWorkflow is None:
            result["step5_status"] = "SKIPPED"
            result["step5_error"] = "Level3GitHubWorkflow import failed; skipping PR creation"
            logger.warning("[Step11] {}", result["step5_error"])
            result["step5_execution_time_ms"] = (time.time() - step_start) * 1000
            return result
        ...
        raw_issue_id = state.get("step2_issue_id", "0") or "0"
        try:
        ...
        # --- Extract branch name ---
        branch_name: str = state.get("step3_branch_name", "") or ""
        if not branch_name:
            result["step5_status"] = "SKIPPED"
            result["step5_error"] = "step3_branch_name is empty; cannot create PR"
            logger.warning("[Step11] {}", result["step5_error"])
            result["step5_execution_time_ms"] = (time.time() - step_start) * 1000
            return result

        # --- Build changes summary string ---
        changes_summary_str: str = state.get("step4_implementation_summary", "") or ""
        if not changes_summary_str:
            # Fall back to list of modified files
            modified: List[str] = (
                state.get("step4_modified_files", [])
                or (state.get("step4_changes_summary") or {}).get("files_modified", [])
                or []
            )
            if modified:
        ...
        selected_skills: List[str] = state.get("step1_selected_skills", []) or []
        selected_agents: List[str] = state.get("step1_selected_agents", []) or []
        ...
        pr_result: Dict[str, Any] = github_wf.step5_create_pull_request(
        ...
        result["step5_pr_id"] = str(pr_number)
        result["step5_pr_url"] = pr_url
        result["step5_merged"] = merged
        result["step5_review_passed"] = success
        result["step5_status"] = "OK" if success else "ERROR"

        if not success:
            err = pr_result.get("error", "PR creation returned success=False")
            result["step5_error"] = err
            result["step5_review_issues"] = [err]
        ...
        result["step5_error"] = str(exc)
        result["step5_status"] = "ERROR"
        result["step5_review_issues"] = [str(exc)]
    ...
    logger.info(
        ...
        result["step5_status"],
        result["step5_pr_id"],
        result["step5_execution_time_ms"],
    )
    return result
