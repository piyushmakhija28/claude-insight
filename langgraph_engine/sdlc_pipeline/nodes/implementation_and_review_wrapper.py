...
try:
    from .issue_and_branch_wrapper import _build_retry_history_context
except ImportError:
...
try:
    from .implementation_and_review_implementation import step4_implementation_execution, step5_pull_request_review
except ImportError as _imp_err:  # pragma: no cover
    logger.error("implementation_and_review_implementation import failed: {}", _imp_err)

    def step4_implementation_execution(state):  # type: ignore[misc]
        """Fallback stub used when implementation_and_review_implementation import fails."""
        return {
            ...
            "step4_error": "implementation_and_review_implementation unavailable",
        }

    def step5_pull_request_review(state):  # type: ignore[misc]
        """Fallback stub used when implementation_and_review_implementation import fails."""
        return {
            "step5_pr_id": "0",
            "step5_pr_url": "",
            "step5_review_passed": True,
            "step5_review_issues": ["implementation_and_review_implementation unavailable"],
            "step5_merged": False,
            "step5_status": "ERROR",
            "step5_error": "implementation_and_review_implementation unavailable",
        }


def step4_implementation_note(state: FlowState) -> Dict[str, Any]:
    ...
    if jira_key and state.get("jira_issue_created", False):
        try:
            from ..jira_lifecycle import Level3JiraWorkflow

            jira_wf = Level3JiraWorkflow()
            jira_wf.step4_start_progress(jira_issue_key=jira_key)
    ...
    if figma_key and state.get("figma_enabled", False):
        try:
            ...
            figma_wf.step4_implementation_started(
                file_key=figma_key,
    ...
    try:
        ...
        target_files = state.get("step1_files_affected", []) or state.get("step1_target_files", [])
    ...
    retry_count = state.get("step5_retry_count", 0)
    history = _build_retry_history_context(state)
    has_retry = retry_count > 0

    # Build execution prompt (with retry history if applicable)
    base_prompt = state.get("step1_execution_prompt", "")
    if has_retry:
        issues = state.get("step5_review_issues", [])
    ...
    def _with_llm_fallback(st):
        ...
        """
        try:
            result = step4_implementation_execution(st)
            # Track files modified by implementation step
            if result and result.get("step4_modified_files"):
                infra = get_infra(st)
                if infra["metrics"]:
                    try:
                        infra["metrics"].record_files_modified(
                            step=10,
                            files=result["step4_modified_files"],
                            operation="modified",
                        )
                    except Exception as exc:
    ...
    result["step4_execution_prompt"] = exec_prompt
    result["step4_has_retry_context"] = has_retry
    result["step4_status"] = result.get("step4_status", result.get("step4_implementation_status", "OK"))
    result["step4_message"] = result.get("step4_message", "Step 10 executed (retry=%d)" % retry_count)

    # Merge CallGraph data into result for Step 11
    if pre_change_graph:
        result["step4_pre_change_graph"] = pre_change_graph
    if call_context.get("call_graph_available"):
        result["step4_call_context"] = call_context
    if suggested_tests:
        result["step4_suggested_test_scope"] = suggested_tests
    if _dep_result is not None:
        result["step4_dependency_resolution"] = _dep_result
        ...
        if modified_files and os.environ.get("CLAUDE_DRY_RUN") != "1":
            ...
            if scan_result.get("findings"):
                try:
                    ...
                    result["step4_auto_fix_result"] = fix_result
                    logger.info(
            ...
            try:
                ...
                result["step4_generated_tests"] = test_result
            ...
            try:
                ...
                result["step4_generated_integration_tests"] = integ_result
            ...
            try:
                ...
                result["step4_coverage_results"] = coverage_result
            except Exception as e:
    ...
    # snapshots (step1_impact_analysis, pre_analysis_result) are no longer
...
def step5_pull_request_node(state: FlowState) -> Dict[str, Any]:
    ...
    def _with_network_retry(st):
        ...
        for attempt in range(3):
            try:
                return step5_pull_request_review(st)
            except Exception as exc:
    ...
    result = _run_step(
        ...
        state,
        fallback_result={
            "step5_review_passed": True,  # Allow pipeline to continue on error
            "step5_review_issues": ["Step 11 failed - skipping review"],
            "step5_retry_count": state.get("step5_retry_count", 0),
            "step5_status": "ERROR",
        },
    )
    ...
    if jira_key and state.get("jira_issue_created", False):
        try:
            ...
            jira_wf = Level3JiraWorkflow()
            link_result = jira_wf.step5_link_pr_and_transition(
                jira_issue_key=jira_key,
                pr_url=result.get("step5_pr_url", ""),
                pr_number=int(result.get("step5_pr_id", "0")),
            )
    ...
    if result.get("step5_merged") and jira_key:
        try:
            from ..jira_lifecycle import Level3JiraWorkflow

            jira_wf = Level3JiraWorkflow()
            jira_wf.step5_post_merge_update(
                jira_issue_key=jira_key,
                pr_number=int(result.get("step5_pr_id", "0")),
                pr_url=result.get("step5_pr_url", ""),
    ...
    try:
        ...
        project_root = state.get("project_root", ".")
        modified_files = result.get("step4_modified_files") or state.get("step4_modified_files", [])
        pre_snapshot = state.get("step4_pre_change_graph", {})

        if modified_files:
            impact_review = review_change_impact(project_root, modified_files, pre_snapshot)
            if impact_review.get("call_graph_available"):
                result["step5_impact_review"] = impact_review
                result["step5_risk_assessment"] = impact_review.get("risk_assessment", "safe")
                breaking = impact_review.get("breaking_changes", [])
                if breaking:
                    result["step5_breaking_changes"] = breaking
                    # Add breaking changes to review issues
                    existing_issues = result.get("step5_review_issues", [])
                    ...
                    result["step5_review_issues"] = existing_issues
    ...
    try:
        ...
        result["step5_quality_gate"] = gate_result
        ...
        if not gate_result.get("gate_passed", True):
            ...
            result["step5_quality_gate_passed"] = True
    ...
    try:
        from ...user_interaction import InteractionManager, generate_step5_questions

        questions = generate_step5_questions({**state, **result})
        if questions:
            ...
            if os.environ.get("CLAUDE_HOOK_MODE", "1") == "1":
                mgr.apply_defaults()
            result["step5_interaction_questions"] = len(questions)
    except Exception as e:
    ...
    if os.environ.get("ENABLE_FIGMA", "0") == "1":
        ...
        if file_key:
            try:
                ...
                figma_wf = Level3FigmaWorkflow()
                impl_summary = result.get("step5_review_summary", "") or state.get("step4_implementation_summary", "")
                review_result = figma_wf.step5_design_review(
                    file_key=file_key,
