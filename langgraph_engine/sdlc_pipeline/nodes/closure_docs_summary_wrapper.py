...
try:
    from .closure_docs_summary_implementation import (
        step6_issue_closure,
        step7_project_documentation_update,
        step8_final_summary_generation,
    )
except ImportError as _imp_err:  # pragma: no cover
    logger.error("closure_docs_summary_implementation import failed: {}", _imp_err)

    def step6_issue_closure(state):  # type: ignore[misc]
        """Fallback stub used when closure_docs_summary_implementation import fails."""
        return {
            "step6_issue_closed": False,
            "step6_status": "ERROR",
            "step6_error": "closure_docs_summary_implementation unavailable",
        }

    def step7_project_documentation_update(state):  # type: ignore[misc]
        """Fallback stub used when closure_docs_summary_implementation import fails."""
        return {
            "step7_updates_prepared": False,
            "step7_documentation_status": "ERROR",
            "step7_error": "closure_docs_summary_implementation unavailable",
        }

    def step8_final_summary_generation(state):  # type: ignore[misc]
        """Fallback stub used when closure_docs_summary_implementation import fails."""
        return {
            "step8_status": "ERROR",
            "step8_error": "closure_docs_summary_implementation unavailable",
        }


def step6_issue_closure_node(state: FlowState) -> Dict[str, Any]:
    ...
    if jira_key and state.get("jira_issue_created", False):
        try:
            ...
            close_result = jira_wf.step6_close_jira_issue(
                jira_issue_key=jira_key,
                pr_number=int(state.get("step5_pr_id", "0")),
                files_modified=state.get("step4_modified_files", []),
                approach_taken=state.get("step6_closing_comment", ""),
            )
    ...
    if figma_key and state.get("figma_enabled", False):
        try:
            ...
            figma_wf.step6_implementation_complete(
                file_key=figma_key,
                pr_number=int(state.get("step5_pr_id", "0")),
...
def step7_docs_update_node(state: FlowState) -> Dict[str, Any]:
    """Step 13: Documentation Update with file-op error handling."""

    def _with_file_error_handling(st):
        try:
            result = step7_project_documentation_update(st)
            # Track documentation files updated
            if result and result.get("step7_updates_prepared"):
                updated = result.get("step7_updated_files") or []
                if updated:
                    infra = get_infra(st)
                    if infra["metrics"]:
                        try:
...
def step8_final_summary_node(state: FlowState) -> Dict[str, Any]:
    """Step 14: Final Summary with metrics print and full error handling."""

    def _with_metrics_summary(st):
        result = step8_final_summary_generation(st)
        # Print metrics summary at end of pipeline
        infra = get_infra(st)
        if infra["metrics"]:
            try:
                # Record any files modified from state
                modified_files = st.get("step4_modified_files") or st.get("step7_updated_files") or []
                if modified_files:
                    infra["metrics"].record_files_modified(
                        step=14,
