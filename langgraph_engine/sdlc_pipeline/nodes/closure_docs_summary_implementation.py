...
in closure_docs_summary_wrapper.py invoke.
...
def step6_issue_closure(state: FlowState) -> Dict[str, Any]:
    ...
    Returns step6_* keys including issue_closed, closing_comment, and status.
    """
    try:
        # Skip if no real issue exists
        if not state.get("step2_issue_created", False) or state.get("step2_issue_id", "0") == "0":
            logger.info("Step 12: Skipping issue closure -- no issue was created")
            return {
                "step6_issue_closed": False,
                "step6_closing_comment": "",
                "step6_status": "SKIPPED",
            }

        issue_id = state.get("step2_issue_id", "0")
        pr_id = state.get("step5_pr_id", "0")
        pr_url = state.get("step5_pr_url", "")
        review_passed = state.get("step5_review_passed", False)
        ...
        task_type = state.get("step1_task_type", "Task")
        ...
        if Level3GitHubWorkflow is not None:
            try:
                ...
                if result.get("success"):
                    ...
                    if branch_name and state.get("step5_merged", False):
                        try:
                    ...
                    return {
                        "step6_issue_closed": True,
                        "step6_closing_comment": "Issue #%s closed via PR #%s" % (issue_id, pr_id),
                        "step6_status": "OK",
                    }
                else:
        ...
        return {
            "step6_issue_closed": False,
            "step6_closing_comment": closing_comment,
            "step6_status": "FALLBACK",
        }

    except Exception as e:
        return {"step6_issue_closed": False, "step6_status": "ERROR", "step6_error": str(e)}
...
def step7_project_documentation_update(state: FlowState) -> Dict[str, Any]:
    ...
    Returns step7_* keys including updates_prepared, updated_files, and status.
    """
    try:
            ...
            try:
                ...
                content = (
                    ...
                    % (
                        ...
                        task_type,
                        complexity,
                        result.get("step7_documentation_status", "OK"),
                        ", ".join(result.get("step7_updated_files", [])),
                    )
                )
        ...
        return {
            "step7_updates_prepared": True,
            "step7_update_count": len(result.get("step7_updated_files", [])),
            "step7_updated_files": result.get("step7_updated_files", []),
            "step7_documentation_status": result.get("step7_documentation_status", "OK"),
            "step7_docs_created": result.get("step7_docs_created", []),
            "step7_uml_diagrams": uml_diagrams_generated,
        }

    except Exception as e:
        return {
            "step7_updates_prepared": False,
            "step7_documentation_status": "ERROR",
            "step7_error": str(e),
        }
...
def step8_final_summary_generation(state: FlowState) -> Dict[str, Any]:
    ...
    try:
        ...
        issue_created = state.get("step2_issue_created", False)
        issue_url = state.get("step2_issue_url", "")
        pr_url = state.get("step5_pr_url", "")
        pr_merged = state.get("step5_merged", False)
        modified_files = state.get("step4_modified_files", [])
        ...
        result = {
            ...
            "step8_summary_saved": summary_saved,
            "step8_voice_sent": voice_sent,
            "step8_status": "OK",
        }
        ...
        return {"step8_status": "ERROR", "step8_error": str(e)}
