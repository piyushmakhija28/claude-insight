...
def _evaluate_sonar_gate(
    ...
    try:
        ...
        if sonar_data is None:
            for key in ("step4_findings", "step5_findings"):
                candidate = state.get(key)
...
def _evaluate_coverage_gate(
    ...
    try:
        ...
        for key in (
            "step4_coverage_results",
            "step5_coverage_results",
            "step4_test_scope",
        ):
            candidate = state.get(key)
            if candidate and isinstance(candidate, dict):
...
def _evaluate_breaking_changes_gate(
    ...
    Reads step5_breaking_changes and step5_risk_assessment from state.
    ...
    try:
        ...
        breaking_changes: List[Dict[str, Any]] = state.get("step5_breaking_changes") or []
        risk_assessment: str = state.get("step5_risk_assessment") or "safe"

        # Also check step5_impact_review for additional data
        impact_review = state.get("step5_impact_review") or {}
...
def _evaluate_tests_exist_gate(
    ...
    try:
        ...
        gen_result = state.get("step4_generated_tests") or state.get("step5_generated_tests") or {}
...
def _evaluate_faithfulness_gate(
    ...
    try:
        ...
        task_description = state.get("user_message") or state.get("step1_execution_prompt") or ""
        modified_files: List[str] = list(state.get("step4_modified_files") or [])
